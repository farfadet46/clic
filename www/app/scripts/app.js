...
app.beep();
      ...
      methods: {
        ...
        swipeDownHandler: function (event) {
          var
            view = this,
            self = this,
            views = app._getLoadedViews(),
...
window.app = new Vue({
  ...
  router: new VueRouter({routes: [
    ...
    { path: '/view1', alias: '*', name: 'view1', component: window.appViews.view1 }
  ]}),
  ...
  methods: {
    ...
    setAppTheme: function (themeName) {
      $('#app-theme').attr('href',
       'app/styles/' + themeName.toLowerCase() + '.css');
    },
    ...
    setViewReadyForDeviceKeyboard: function (viewName) {
      if (window.cordova) {
        $('#' + viewName).css({
          "margin-top": '0px',
    ...
    webExtensionSendMessage: function (message, callback) {
      if (window.chrome) {
    ...
    _appEventSwipeUp: function (event) {
      var
        self = this,
        view = app._getCurrentView(),
        views = app._getLoadedViews(),
    ...
    _appEventSwipeDown: function (event) {
      var
        self = this,
        view = app._getCurrentView(),
        views = app._getLoadedViews(),
    ...
    _setupAppDocumentEvents: function () {
      ...
      window.document.addEventListener('keyup', this._appEventOnKeuUp);
    ...
    _setupAppHammerEvents: function () {
      ...
      ha.get('swipe').set({direction: Hammer.DIRECTION_ALL});
      ha.on('swipeup', this._appEventSwipeUp);
      ha.on('swiperight', this._appEventSwipeRight);
      ha.on('swipedown', this._appEventSwipeDown);
    ...
    _setViewEvents: function (view) {
      ...
      ha.get('swipe').set({direction: Hammer.DIRECTION_ALL});
      ha.on('swipeup', view.swipeUpHandler);
      ha.on('swiperight', view.swipeRightHandler);
      ha.on('swipedown', view.swipeDownHandler);
